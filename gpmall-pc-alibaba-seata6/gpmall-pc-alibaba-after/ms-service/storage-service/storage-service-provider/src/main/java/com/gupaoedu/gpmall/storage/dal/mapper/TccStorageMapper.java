...
public interface TccStorageMapper extends BaseMapper<TccStorage> {
    ...
    @Update("update tcc_storage set frozen_count=frozen_count-#{count} where commodity_code=#{commodityCode}")
    int decrementStorageCommit(@Param("count")Integer count,@Param("commodityCode")String commodityCode);

    @Update("update tcc_storage set count=count+#{count},frozen_count=frozen_count-#{count} where commodity_code=#{commodityCode}")
    int decrementStorageCancel(@Param("count")Integer count,@Param("commodityCode")String commodityCode);
}
