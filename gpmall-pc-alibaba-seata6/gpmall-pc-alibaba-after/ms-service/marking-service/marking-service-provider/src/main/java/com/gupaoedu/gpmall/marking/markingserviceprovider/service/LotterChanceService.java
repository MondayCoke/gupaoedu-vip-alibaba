...
public class LotterChanceService implements ILotterChanceService {
    ...
    @Override
    public UserDrawChanceResponse incrementUserDrawChance(UserDrawChanceRequest request) {
        log.info("[UserDrawChanceResponse.incrementUserDrawChance], request : {}",request);
        UserDrawChanceResponse response=new UserDrawChanceResponse();
        try{
            request.checkParam();
            ...
            lotteryChanceMapper.insert(lotteryChance);
            response.setCode(MmsResCodeEnum.SYS_SUCCESS.getCode());
            response.setMsg(MmsResCodeEnum.SYS_SUCCESS.getMsg());
        }catch (DuplicateKeyException de){
            //存在则更新
            int rows=lotteryChanceMapper.incrementLotteryChance(request.getUid());
            if(rows>0){
                response.setCode(MmsResCodeEnum.SYS_SUCCESS.getCode());
                response.setMsg(MmsResCodeEnum.SYS_SUCCESS.getMsg());
                return response;
            }
            response.setCode(MmsResCodeEnum.SYS_UPDATE_DATA_FAIL.getCode());
            response.setMsg(MmsResCodeEnum.SYS_UPDATE_DATA_FAIL.getMsg());
        }catch (Exception e){
            log.error("[UserDrawChanceResponse.incrementUserDrawChance], occur Exception",e);
            MmsExceptionWrapper.handlerException4biz(response,e);
        }
        return response;
    }
